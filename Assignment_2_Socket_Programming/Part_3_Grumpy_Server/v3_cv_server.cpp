...
struct ServerState {
    ...
    int current_client_number = -1;
    std::chrono::time_point<std::chrono::system_clock> request_start_time;
    std::chrono::time_point<std::chrono::system_clock> last_collision_time;
};
    ...
    while (true)
    {
        ...
        // Lock the mutex to check and update server state
 
         {
            ...
            // If the server is busy, check for collision
            if (server_state.server_busy && server_state.current_socket_id != data_socket) {
                // Log collision
                server_state.last_collision_time = std::chrono::system_clock::now();
                server_log << "Collision detected for client " << client_number << " with previously running client: "<<server_state.current_client_number<<"  . Waiting..." << endl;

                // Wait for the server to become available
                cv.wait(lock);
            }

            // Mark server as busy for this client
            server_state.server_busy = true;
            server_state.current_socket_id = data_socket;
            server_state.current_client_number = client_number;
            server_state.request_start_time = std::chrono::system_clock::now();
        }
        ...
        {
            std::unique_lock<std::mutex> lock(collision_mutex);
            server_state.server_busy = false;
            server_state.current_socket_id = -1;
            server_state.current_client_number = -1;
            cv.notify_all();
        }
    }
