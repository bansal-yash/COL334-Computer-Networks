...
bool send_server_to_client_msgs(int data_socket, int k, int p, int client_number, int offset_value, ofstream &server_log)
{
    ...
    if (memory_size - offset_value >= k)
    {
        ...
        while (num_packets > 0)
        {
            ...
            if(dict[client_number] == true){
                cout<<"^^^^^^^^Collision found in the send_server function for client: "<<client_number<<endl;
                dict[client_number] = false;
                break; 
            }
            words_to_send += '\n';
...
void handle_client(int data_socket, int k, int p, int client_number)
{
    ...
    while (true)
    {
        ...
        if(is_busy == true && client_number != current_client){
            dict[client_number] = true;
            dict[current_client] = true;
            ...
            cout <<"SEIZING packets sent to : "<<client_number<<endl;
            cout<<"Collsion has occured !! ============ between : new_client : "<<client_number<<" and old client: "<<current_client<<endl;
            if(strlen(msg_array) > 0){
                cout << "\t HUH! message sent to newly requested client : "<<data_socket<< " num: "<<client_number<<endl;
                cout << "\t HUH! message sent to previously client which made server busy : "<<current_client_socket<<" num: "<<current_client<<endl;
            }
            ...
            auto time_diff = std::chrono::duration_cast<std::chrono::seconds>(now - start_time).count();
            cout<<"=;=;=;=;=;=;=;= TIME LEFT IN THE SLOT: "<<time_diff<<endl;
            sleep(time_diff);

            cout<<"slept for "<<time_diff<<" seconds"<<endl;          
        }else{
            is_busy = true;
            current_client = client_number;
            current_client_socket = data_socket;
            start_time = now;
            auto diff_from_Start = std::chrono::duration_cast<std::chrono::seconds>(start_time - global_start).count();
            cout<<"Current start time for the slot: "<<diff_from_Start<<endl;
            // pthread_mutex_unlock(&request_mutex);
