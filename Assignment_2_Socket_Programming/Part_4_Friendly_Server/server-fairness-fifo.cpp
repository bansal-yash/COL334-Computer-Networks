...
vector<pair<int, int>> pending_requests;
...
vector<int> get_offset_fair(string offset_str)
{
    string s = "";
    vector<int> temp;
    for (int i = 0; i < offset_str.size(); i++)
    {
        if (offset_str[i] == '\n')
        {
            int kk = stoi(s);
...
void handle_client(int data_socket, int k, int p, int client_number)
{
    ...
    while (true)
    {
...

        {
            ...
            for (int offset_val : offsets)
            {
                server_log << "Offset recieved:- " << offset_val << endl;
                pending_requests.push_back({client_number, offset_val});
            }
            offsets = {};
            cv.wait(lock, [client_number]()
                    { return pending_requests.front().first == client_number; });
        }

        offset_value = pending_requests.front().second;
        ...
        num_send++;
        {
            ...
            if (eof && rogue)
            {
                while (!pending_requests.empty() && (pending_requests.front().first == client_number))
                {
                    pending_requests.erase(pending_requests.begin());
                }
            }
